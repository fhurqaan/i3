...
# jsonPath=$HOME/<directory path to save the layout json files>
# ie: jsonPath="$HOME/.config/i3/"
# jsonPath="$HOME/.config/i3/layouts/"
...
echo -e "\e[1;93m"JSON file will be saved in"\e[1;95m" $jsonPath"\e[0m"
for i in ${!ws[@]}; do
	# skip elenment 0
	if (( $i == 0 )); then
		continue
	...
	if [ $lines -gt 1 ]; then
		...
		echo "   $wsname is unused....skipping"
		rm $jsonFile
	fi
done
